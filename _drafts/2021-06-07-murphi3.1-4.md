...
This is the continuation of [previous post](https://amutheezan.com/formal%20methods%20and%20verification/murphi3.1-3/)
which provides a toy example in Murphi3.1.

In this post, I will go through small extension to toy example using murphi3.1, based on what I have discussed in
earlier posts.

### SMALL EXTENSION TOY-EXAMPLE - Simple File Management System

This example is adapted from [MSI Protocol](https://en.wikipedia.org/wiki/MSI_protocol).

We extend the File Management System which we explain in the previous post, to support either multiple readers can read
the file or only one writer can write the file with less bus transactions. We define this model by a simple Finite state
machine with following states,
...
In this model, we split the ```Used``` states to two states ```Shared``` and ```Modified```.
...
![image](https://amutheezan.com/images/3states.png)
...
In the [next post](https://amutheezan.com/formal%20methods%20and%20verification/murphi3.1-5/), I will talk about how to
fully extend this this extended file system.
